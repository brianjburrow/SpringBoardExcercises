...
function vowelCount(str) {
    let vowels = 'aeiou';
    let counts = {};
    Array.from(str.toLowerCase()).forEach((value) => {
        if (vowels.includes(value)) {
            if (!counts[value]) { counts[value] = 1 } else { counts[value]++ }
        }
    })
    return counts;
}
...
returns a new array with the value of the key with a name of "first" 
...
Examples:
    filterByValue([{first: 'Elie', last:"Schoppik"}, {first: 'Tim', last:"Garcia",
     isCatOwner: true}, {first: 'Matt', last:"Lane"}, {first: 'Colt', last:"Steele", isCatOwner: true}], 'isCatOwner') // [{first: 'Tim', last:"Garcia", isCatOwner: true}, {first: 'Colt', last:"Steele", isCatOwner: true}]
*/
...
function findInObj(arr, key, searchValue) {
    for (object of arr) {
        if (object[key] && object[key] == searchValue) return object;
    }
}
...
function removeVowels(str) {
    return Array.from(str.toLowerCase()).filter((value) => {
        if (!'aeiou'.includes(value)) { return true }
    }).join('')
}
