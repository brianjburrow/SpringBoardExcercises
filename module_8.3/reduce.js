...
function extractValue(arr, key) {
    return arr.reduce((list, object) => {
        if (object[key]) { list.push(object[key]); }
        return list
    }, [])
}
...
function vowelCount(str) {
    return Array.from(str).reduce((counter, character) => {
...
function partition(arr, callback) {
    return arr.reduce((collector, value) => {
        if (callback(value)) { collector[0].push(value) } else { collector[1].push(value) }
        return collector;
    }, [[], []]);
}
