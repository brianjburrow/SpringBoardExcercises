...
function hasAZero(num) {
    return Array.from(num.toString()).some((value) => {
...
values (more than one element in the array that has the same value as another). 
...
function hasNoDuplicates(arr) {
    const set = [];
    return arr.every((value) => {
        console.log(set, value, set.includes(value))
...
function hasCertainValue(arr, key, searchValue) {
    let ans = arr.every((object) => {
        if (object[key] && object[key] === searchValue) { return true }
    })
    return ans
}
